...
if (-not (Test-Path $FrontendDir)) { throw "Missing ./frontend directory (expected at $FrontendDir)" }

# Optional env overrides:
#   $Env:PREVIEW_URL_FALLBACK  -> e.g. https://your-preview.vercel.app
#   $Env:VERCEL_BYPASS_TOKEN   -> for protected previews
#   $Env:VERCEL_TOKEN          -> to look up preview via Vercel API
$PreviewUrlFallback = $Env:PREVIEW_URL_FALLBACK
if (-not $PreviewUrlFallback) { $PreviewUrlFallback = 'https://your-preview.vercel.app' }  # <-- set me
...
# --- 0) Ignore & untrack large artifacts (keep files on disk) --------
...
if ($toAdd.Count -gt 0) {
  ...
  Add-Content $gitignore ($toAdd -join "`n"); Add-Content $gitignore ""
}
# untrack known offenders and any tracked files >= 95MB
& $GitExe rm -r --cached --ignore-unmatch -- '_artifacts' 2>$null | Out-Null
$tracked = (& $GitExe ls-files 2>$null) -split "`n"
$big=@(); foreach($p in $tracked){ if ($p){
  try { $fi=Get-Item -LiteralPath $p -ErrorAction SilentlyContinue; if ($fi -and $fi.Length -ge 95MB){ $big += $p } } catch {}
}}
foreach($p in $big){ & $GitExe rm --cached -- "$p" 2>$null | Out-Null }
if ($big.Count -gt 0){ Note "Untracked big files from index: $($big.Count)" }

# --- 1) Normalize robots + middleware (idempotent) -------------------
...
# middleware: add X-Robots-Tag off-prod
...
# next-sitemap (safe even if route wins)
...
# minimal sitemap placeholder (only if missing)
...
# --- 2) Commit & push (with soft reset if needed) --------------------
...
if ($insideRepo) {
  ...
  $ahead = 0
  try {
    $counts = (& $GitExe rev-list --left-right --count "origin/$branch...$branch" 2>$null).Trim()
    if ($counts -match '^\s*\d+\s+(\d+)\s*$'){ $ahead = [int]$Matches[1] }
  } catch {}
  if ($ahead -gt 0 -and ($big.Count -gt 0 -or (Test-Path '_artifacts'))) {
    & $GitExe reset --soft "origin/$branch" 2>$null | Out-Null
    Note "Soft-reset to origin/$branch to drop large-file commits."
  }
...
} else { Note "Not a git repo — skipping commit/push." }
  ...
  if (Test-Path $vercelMetaFile) {
    ...
    if ($projectId) {
      ...
      do {
        ...
        $resp = HttpGet("https://api.vercel.com/v6/deployments?projectId=$projectId&target=preview&limit=50")
        if (-not $resp) { break }
        $candidates = @()
        foreach($d in $resp.deployments){
          ...
          foreach($nm in @('githubCommitRef','gitlabCommitRef','bitbucketCommitRef','commitRef','branch')){
            if ($meta.PSObject.Properties.Name -contains $nm) { if (("$($meta.$nm)" -eq $branch)) { $branchMatch = $true; break } }
          }
          if ($branchMatch) { $candidates += $d }
        }
        if (-not $candidates) { continue }
        $exact = $candidates | Where-Object { Matches-SHA $_.meta $sha }
        $pick  = if ($exact) { $exact | Sort-Object created -Descending | Select-Object -First 1 } else { $candidates | Sort-Object created -Descending | Select-Object -First 1 }
...
try {
  ...
  if ($BypassToken) {
    Invoke-WebRequest "$previewUrl/?x-vercel-set-bypass-cookie=true&x-vercel-protection-bypass=$BypassToken" `
      -WebSession $session -MaximumRedirection 20 -UseBasicParsing | Out-Null
  }
  $cb = (Get-Date).Ticks
  ...
  if ($robots -match '(?im)^\s*Disallow:\s*/\s*$' -and ($xrt -match 'noindex')) {
    Write-Host "`n√ Preview is not indexable (robots + header OK) - $previewUrl" -ForegroundColor Green
  } else {
    Write-Warning "`nPreview still shows old robots; verify Vercel Root Directory=frontend or redeploy latest commit."
  }
} catch {
