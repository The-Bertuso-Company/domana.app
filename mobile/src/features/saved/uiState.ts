...
export type SortBy =
  ...
  | "psf_asc"
  | "beds_desc" | "baths_desc" | "sqft_desc"
  | "distance_asc";
...
export const useSavedUi = create(
  persist<UiState>(
    (set, get) => ({
      ...
      enterSelect: (initialId) => set((state) => {
        const next = new Set(state.selectedIds);
        if (initialId) next.add(initialId);
        return { selectMode: true, selectedIds: next };
      }),
    ...
    {
      name: "domana:saved:ui",
      version: 1,
      storage: createJSONStorage(() => AsyncStorage),
      partialize: (s) => ({ sortBy: s.sortBy, filters: s.filters }),
    }
  )
);
