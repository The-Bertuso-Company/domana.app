...
create or replace function enc(p_text text) returns bytea language sql stable strict as $$
...
create or replace function dec(p_bytes bytea) returns text language sql stable strict as $$
  select case when coalesce(current_setting('app.crypto_key', true),'') <> ''
              then pgp_sym_decrypt(p_bytes, current_setting('app.crypto_key', true))
              else null::text end
$$;

create or replace function enc_key(p_text text, p_key text) returns bytea language sql stable strict as $$
  select pgp_sym_encrypt(p_text, p_key)
$$;

create or replace function dec_key(p_bytes bytea, p_key text) returns text language sql stable strict as $$
  select pgp_sym_decrypt(p_bytes, p_key)
$$;
...
-- smoke test, returns one row: crypto_ok = t
select dec(enc('ok'))='ok' as crypto_ok;
`;

  try {
    await db.query(sql);
    console.log("Phase-4 SQL applied ✅");
  } catch (e) {
